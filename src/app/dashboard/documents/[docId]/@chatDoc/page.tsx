...
export default function ChatProj() {
  ...
  const messages = useQuery(api.messages.getAllMessages, {
    recordId: docId as Id<"documents">,
    userId: user?.id ?? "",
  });
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const question = formData.get("doc_message");
    await sendMessage({
      isHuman: true,
      recordId: docId as Id<"documents">,
    ...
    const { output } = await askDocQuestion(question as string, docId, user.id);
                    ...
                    <div className="flex gap-2">
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger
                            onClick={() => {
                              addMessageToNote(message.text, currentDoc.title);
                            }}
                          >
      ...
      <div className="border-t border-gray-100 dark:border-gray-600 p-4 bg-white dark:bg-gray-700 rounded-md">
        ...
        >
          <Textarea
            ...
            placeholder={`Ask anything about ${currentDoc.title}...`}
