...
function DeleteDialog({
  ...
  const deleteHandler = useMutation(api.video.deleteVideoById);
  const handleDelete = async () => {
    const deleteVideo = deleteHandler({
      id: videoId,
      userId,
    })
      .then(() => {
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            Are you sure to delete video : {videoTitle} ?
          </AlertDialogTitle>
          <AlertDialogDescription>
...
export default function Videos() {
  ...
  const videos = useQuery(api.video.getAllVideos);
